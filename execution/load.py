...
class OutlineNode(object):
    ...
    def traversal_print(self):
        # traversal prints the whole nodes' tree
        print(self)
...
def read_opml(location):
    ...
    def analyse_opml(opml_content):
        """Analyses the opml codes and builds a nodes' tree for it. Noted that the basic element of the nodes' tree
        is OutlineNode.
        ...
        for content in opml_content:
            ...
            node_split = re.split('<outline text="|" _note="|" heading="1"|" heading="2"|" heading="3"|"/>|">|>',
                                  content)
            ...
            # if the layer of the new node is smaller than the current node,
            # then find the node in the same layer with the new node from the current node's fathers series,
            # they should share the same father
            if layer < current_node.layer:
...
def read_docx(path):
    ...
    def sorting_docx(location):
        ...
        # distinguish the titles and the corresponding notes and the layer they belong to
        titles = []
        notes = []
        layers = []
    ...
    nodes = sorting_docx(path)
    root_node = analyse_docx(nodes)
    return root_node
...
def load_files(path):
    ...
    Args:
        path: A string that contains the folder location of the target file.
    ...
    # load the files according to its format
    root_node_list = []
    filename_list = []
    for file in file_list:
        if '.opml' in file:
            root_node_list.append(read_opml(path + file))
            ...
            if '~$' in file:  # which indicates that this is a temporary file of no value
                break
            root_node_list.append(read_docx(path + file))
            filename_list.append(file)

        else:
            pass

    return root_node_list, filename_list
